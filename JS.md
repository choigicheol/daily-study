...
**_실제 자바스크립트가 구동되는 환경(브라우저, Node.js 등)에서는 주로 여러 개의 스레드가 사용_**되는데, 이러한 구동 환경이 단일 호출 스택을 사용하는 자바 스크립트 엔진과 연동하기 위해 사용되는 장치가 바로 '이벤트 루프' 이다.

핵심만 말하자면, JavaScript event loop는 call stack이 비어있는 경우, callback queue에서 대기하던 callback을 call stack으로 옮겨서 실행시켜주는 역할을 한다.
...
- 콜백 큐는 선입선출이므로 먼저 들어간 콜백이 먼저 콜스택으로 이동하게 된다. 대기시간을 고려하면 callBack2의 대기시간이 0ms, callBack1이 5000ms이므로 callBack2가 콜스택으로 먼저 이동할 수 있다.
...
setTimeout 함수의 대기시간이 0ms임에도 불구하고 3이 2보다 먼저 콘솔이 찍혀서 나오는 이유는 Callback Queue의 우선순위 때문이다. 이전에도 말한바와 같이 Callback Queue에도 우선순위가 있는데 (Microtask Queue > Animation Frames > Task Queue) 일반적인 작업은 대부분 Task Queue에 쌓인다. setTimeout 역시 Task Queue에 쌓인다. 하지만 프로미스의 경우 Task Queue가 아닌 Microtask Queue에 쌓인다.
...
java script의 코드를 모두 동기적인 방식으로 처리하게되면 순차적으로 처리를 해야하기때문에 중간에 오래걸리는 작업이 있다면 다음 코드로 넘어가지못하고 해당 작업을 완료한 후에 다음 코드를 실행 할 것이다. 그러다 또 오래걸리는 작업에서 멈추고 페이지 하나를 완전히 띄우는데 상당한 시간이 소요될것이다.
...
> 예시를 들자면 집안일을 할 때 세탁기 앞에서 1시간동안 끝나기를 기다렸다가 10분간 옷을 널고, 설거지를 10분간 하고, 방청소를 30분간 한다면 얼마나 비효율적일까? 집안일에 총 1시간 50분이 걸릴테니 말이다.  
> 세탁기가 돌아가는동안 설거지와 방청소를 해놓고 세탁기가 끝나는 시점에 옷을 널어 놓는다면 1시간10분만에 집안일을 다 끝낼 수 있다.

예시에선 비동기 작업에 해당하는 세탁기 작업이 1시간이면 끝날것이라는것을 알수 있지만

아래와 같이 코드상에선 환경적인 요소 등에 의해 비동기 작업에 대한 결과를 받는데까지의 시간을 알 수 없다.
...
그렇기 때문에 작업을 비동기 처리를 했을 떄, 해당 작업의 응답으로 받는 데이터를 이용하여 다른 작업을 이어나가야한다면, 비동기 작업이 끝나자마자 다음 작업에 대한 처리를 해주어야 하는데
...
function foo() {
  ...
  setTimeout(() => {
    name = "choi";
  }, Math.random() * 1000); // 서버에서 데이터를 가져오는 과정(실제로도 얼마나 걸릴지 모른다)
...
setTimeout으로 name의 값을 받아오는작업이 비동기 처리 되고 일정시간뒤에 name값이 할당 되었기때문에 콘솔을 찍어보면 "hello undefined" 가 나온다.(지연시간을 0ms로 해도 마찬가지)

의도한 바는 "hello {name}" 이 나타나는 것이었는데 name의 값을 비동기로 응답받는동안 다음 코드로 넘어가게 되면서 콘솔이 먼저 찍혀 "hello undefined"가 됐다.  
그렇다면 name의 값을 일정시간뒤에 응답받고서 console.log()를 찍게끔하면 의도한대로 동작 할 것이다.
